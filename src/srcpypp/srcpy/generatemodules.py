...
def GetFilenames(rm, isclient=False):
    ...
    path = rm.path
    path = path.split(rm.settings.srcpath)[1][1:]
    if not rm.split:
...
def ParseModules(settings, specificmodule=None, appendfileonly=False):
    ...
    if hasattr(settings, 'vpcserverpath'):
        genvpc.GenerateVPCs(settings, shared_filenames + server_filenames, settings.vpcserverpath)
    if hasattr(settings, 'vpcclientpath'):
        genvpc.GenerateVPCs(settings, shared_filenames + client_filenames, settings.vpcclientpath)

...
void ScrollBarSlider::ApplySchemeSettings(IScheme *pScheme)
{
	...
	for( int i = 0; i < Slider_Count; i++ )
		_ScrollBarSliderBorder[i] = pScheme->GetBorder("ButtonBorder");
}
...
void ScrollBarSlider::Paint()
{
	...
	if (_vertical)
	{
		...
		// border
		if (_ScrollBarSliderBorder[Slider_Idle])
		{
			_ScrollBarSliderBorder[Slider_Idle]->Paint(0, _nobPos[0], wide - 1, _nobPos[1]);
		}
	}
	else
	{
		...
		if (_ScrollBarSliderBorder[Slider_Idle])
		{
			_ScrollBarSliderBorder[Slider_Idle]->Paint(_nobPos[0] - 1, 1, _nobPos[1], tall );
		}
	}
...
{
	VGUI_UNIMPLEMENTED(ScrollBarSlider::IsMouseOverNob);
	return false;
}
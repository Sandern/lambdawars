...
{
	return m_pTreeView->IsItemDroppable( m_ItemIndex, -1, msglist );
}

void TreeNode::OnPanelDropped( CUtlVector< KeyValues * >& msglist )
{
	m_pTreeView->OnItemDropped( m_ItemIndex, -1, msglist );
}
...
{
	if( nSelectionIndex >= 0 && nSelectionIndex < m_SelectedItems.Count() )
...
// Purpose: 
//-----------------------------------------------------------------------------
void TreeView::GetSelectedItemsForDrag( int nPrimaryDagItem, CUtlVector< int >& list )
{
	VGUI_UNIMPLEMENTED(TreeView::GetSelectedItemsForDrag);
}
...
{
	VGUI_UNIMPLEMENTED(TreeView::CanCurrentlyEditLabel);
	return false;
}
...
{
	VGUI_UNIMPLEMENTED(TreeView::FirstItem);
	return -1;
}
...
{
	VGUI_UNIMPLEMENTED(TreeView::OnContextMenuSelection);
}
...
void TextImage::Paint()
{
	...
	for (wchar_t *wsz = _utext; *wsz != 0; wsz++)
	{
		...
		else if (ch == '\n')
		{
			// newline
			iIndent++;
			if ( iIndent < m_LineXIndent.Count() )
			{
...
{
	Warning("Unimplemented function TextImage::SetAllCaps called\n");
}
...
{
	Warning("Unimplemented function TextImage::SetBounds called\n");
}

...
void FileOpenDialog::Init( const char *title, KeyValues *pContextKeyValues )
{
	...
	m_pFileNameCombo = new ComboBox( this, "FileNameCombo", 6, false );
	m_pFileNameCombo->AddActionSignalTarget(this);
...
{
	VGUI_UNIMPLEMENTED(FileOpenDialog::OnMatchStringSelected);
}
...
//-----------------------------------------------------------------------------
void FileOpenDialog::PopulateFileNameSearchHistory()//PopulateFileNameCompletion()
{
	char buf[80];
	//m_pFileNameEdit->GetText(buf, 80);
	m_pFileNameCombo->GetText(buf, 80);
	wchar_t wbuf[80];
	//m_pFileNameEdit->GetText(wbuf, 80);
	m_pFileNameCombo->GetText(wbuf, 80);
	int bufLen = wcslen(wbuf);

	// delete all items before we check if there's even a string
	//m_pFileNameEdit->DeleteAllItems();
	m_pFileNameCombo->DeleteAllItems();
	...
	for ( i = 0 ; i < nCount ; i++ )
	{
		...
		if ( !_wcsnicmp(wbuf, wszString, bufLen) )
		{
			...
			m_pFileNameCombo->AddItem(wszString, NULL);
		}
	}

	// if there are any items - show the menu
	//if ( m_pFileNameEdit->GetItemCount() > 0 )
	if( m_pFileNameCombo->GetItemCount() > 0 )
	{
		//m_pFileNameEdit->ShowMenu();
		m_pFileNameCombo->ShowMenu();
	}
	else
	{
		//m_pFileNameEdit->HideMenu();
	...
	m_pFileNameCombo->InvalidateLayout();
}

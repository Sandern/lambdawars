...
static CUtlLinkedList<Panel*, unsigned short> g_PanelList;
...
BEGIN_DMXELEMENT_UNPACK( Panel )
END_DMXELEMENT_UNPACK( Panel, Panel::s_pUnpackParams )
...
void Panel::Init( int x, int y, int wide, int tall )
{
	...
	m_bPyDeleted = false;
#ifdef USESURFACECALLBUFFER
	m_SurfaceCallRecordFrameTime = -1;
	m_SurfaceCallDrawFrameTime = -1;
	m_bUseSurfaceCallBuffer = false;
	m_bShouldRecordBuffer = false;
	m_bFlushedByParent = false;
#endif // USESURFACECALLBUFFER

	m_bIsDMXSerialized = false;
	m_bIsConsoleStylePanel = false;

	m_NavUp = m_NavDown = m_NavLeft = m_NavRight = NULL;
	m_sNavUpName.Set(NULL);
	m_sNavDownName.Set(NULL);
	m_sNavLeftName.Set(NULL);
	m_sNavRightName.Set(NULL);

	//ivgui()->AssociatePanelWithContext( DEFAULT_VGUI_CONTEXT, GetVPanel() );
	ivgui()->ResetContext( DEFAULT_VGUI_CONTEXT );
	//SetMessageContextId_R( DEFAULT_VGUI_CONTEXT );
}
...
Panel::~Panel()
{
	g_PanelList.FindAndRemove(this);
	...
	if( GetPySelf() )
	{
		...
		// Call special python on delete method
		boost::python::object updateondelete;
		try {
			updateondelete = ref.attr("UpdateOnDelete");
		...
		static boost::python::object _vguicontrols = boost::python::import("_vguicontrols");
	...
	while (ipanel()->GetChildCount(GetVPanel()))
	{
		...
		{
			if( GetChild(0) == NULL || GetChild(0)->IsPythonManaged() == false )
				ipanel()->DeletePanel(child);
			else
				GetChild(0)->PyDeletePanel();
		}
		else
		{
...
// Purpose: 
//-----------------------------------------------------------------------------
void Panel::ShutdownPythonPanels()
{
	FOR_EACH_LL( g_PanelList, i )
	{
		Panel *pPanel = g_PanelList[i];
		if( pPanel->IsPythonManaged() && pPanel->m_bPyDeleted == false )
			pPanel->PyDeletePanel();
	}
	g_PanelList.Purge();
}
...
// Purpose: 
//-----------------------------------------------------------------------------
void Panel::DebugPrintNoneSBufferedPyPanels()
{
#ifdef USESURFACECALLBUFFER
	FOR_EACH_LL( g_PanelList, i )
	{
		...
		if( pPanel->IsPythonManaged() && pPanel->m_bPyDeleted == false )
		{
			if( !pPanel->IsVisible() )
				continue;
			if( !pPanel->IsSBufferEnabled() )
			{
				Msg("Panel %s is not sbuffered\n", pPanel->GetName() );
			}
		}
	}
#endif USESURFACECALLBUFFER
...
//-----------------------------------------------------------------------------
// Free this panel, but don't delete. Python will clean up, but make sure this panel is not visible anymore.
//-----------------------------------------------------------------------------
void Panel::PyDeletePanel(void)
{
	if( m_bPyDeleted )
		return;
...
// 
//-----------------------------------------------------------------------------
void Panel::RegMessageMethod(const char *message, boost::python::object method, int numParams, 
							 const char *nameFirstParam, int typeFirstParam, 
							 const char *nameSecondParam, int typeSecondParam )
{
	if( m_bPyDeleted )
		return;
	...
	entry.nameFirstParam = nameFirstParam;
	entry.typeFirstParam = typeFirstParam;
	entry.nameSecondParam = nameSecondParam;
	entry.typeSecondParam = typeSecondParam;

	m_PyMessageMap.Insert(message, entry);
}

void Panel::PyOnMessage(KeyValues *params, VPANEL fromPanel)
{
	short lookup = m_PyMessageMap.Find(params->GetName());
	if( lookup == m_PyMessageMap.InvalidIndex() )
	{
		// Not handled by the python map
		return;
	}

	py_message_entry_t &entry = m_PyMessageMap.Element(lookup);
	...
	try {
		...
		case 1:
			{
				KeyValues *param1 = params->FindKey(entry.nameFirstParam);
				if (!param1)
				{
					param1 = const_cast<KeyValues *>(params);
				}

				switch ( entry.typeFirstParam )
				{
				case DATATYPE_INT:
				...
				case DATATYPE_PTR:
					if (!stricmp(entry.nameFirstParam, "panel"))
					{
		...
		case 2:
			{
				...
				KeyValues *param2 = params->FindKey(entry.nameSecondParam);
				if (!param2)
				{
					param2 = const_cast<KeyValues *>(params);
				}

				if ( (DATATYPE_INT == entry.typeFirstParam) && (DATATYPE_INT == entry.typeSecondParam) )
				{
					entry.method( param1->GetInt(), param2->GetInt() );
				}
				else if ( (DATATYPE_PTR == entry.typeFirstParam) && (DATATYPE_INT == entry.typeSecondParam) )
				{
					entry.method( param1->GetInt(), param2->GetInt() );
				}
				else if ( (DATATYPE_CONSTCHARPTR == entry.typeFirstParam) && (DATATYPE_INT == entry.typeSecondParam) )
				{
					entry.method( param1->GetString(), param2->GetInt() );
				}
				else if ( (DATATYPE_CONSTCHARPTR == entry.typeFirstParam) && (DATATYPE_CONSTCHARPTR == entry.typeSecondParam) )
				{
					entry.method( param1->GetString(), param2->GetString() );
				}
				else if ( (DATATYPE_INT == entry.typeFirstParam) && (DATATYPE_CONSTCHARPTR == entry.typeSecondParam) )
				{
...
void Panel::PaintTraverse( bool repaint, bool allowForce )
{
...
#ifdef USESURFACECALLBUFFER
	// Surface buffer test
	if( repaint && (!IsSBufferEnabled() || ShouldRecordSBuffer() ) )
#endif // USESURFACECALLBUFFER
	{
...
#ifdef USESURFACECALLBUFFER
	else if( repaint && IsSBufferEnabled() )
	{
		DrawFromSBuffer();
	}
#endif // USESURFACECALLBUFFER
	...
	// draw the border last if we didn't paint it first
	if ( !bBorderPaintFirst && repaint && _flags.IsFlagSet( PAINT_BORDER_ENABLED ) && ( _border != null ) )
	{
		// Paint the border over the background with no inset
		surface()->PushMakeCurrent( vpanel, false );
...
{
	KeyValues *copy = params->MakeCopy();
	PyOnMessage(copy, ifromPanel);		// Let python override
	copy->deleteThis();
	copy = NULL;
...
{
	int lookup = AnimationPropertyConverters().Find( typeName );
	if ( lookup == AnimationPropertyConverters().InvalidIndex() )
		return NULL;

	IPanelAnimationPropertyConverter *converter = AnimationPropertyConverters()[ lookup ];
	return converter;
}
...
void Panel::AddPropertyConverter( char const *typeName, IPanelAnimationPropertyConverter *converter )
{
	...
	AnimationPropertyConverters().Insert( typeName, converter );
}
...
bool Panel::IsSBufferEnabled( void )
{
	return m_bUseSurfaceCallBuffer && !cl_disablesbuffer.GetBool();
}

void Panel::FlushSBuffer( void ) 
{ 
	if( m_bFlushedByParent || cl_sbuffernoflush.GetBool() )
		return;
	m_bShouldRecordBuffer = true; 
}

void Panel::ParentFlushSBuffer( void )
{
...
void Panel::AddSBuffer()
{
	// Add new buffer
	m_SurfaceCallBuffer.AddToTail();
}

bool Panel::ShouldRecordSBuffer()
{
	// Recording this frame, so true (there might be multiple calls to paint during a frame)
	//Msg("m_SurfaceCallRecordTick %d\n", m_SurfaceCallRecordTick);
	if( m_SurfaceCallRecordFrameTime == system()->GetFrameTime() )
	{
		//Msg("Continueing recording sbuffer %s %s\n", GetName(), GetClassName());
		// Start recording
		AddSBuffer();
		SBufferStartRecording( &m_SurfaceCallBuffer.Tail() );
		return true;
	}

	// Decide if we should record or draw from the buffer
	if( !m_bShouldRecordBuffer )
	{
	...
	m_SurfaceCallRecordFrameTime = system()->GetFrameTime();

	for( int i = 0; i < m_SurfaceCallBuffer.Count(); i++ )
		m_SurfaceCallBuffer[i].PurgeAndDeleteElements();
	m_SurfaceCallBuffer.Purge();
	AddSBuffer();
...
bool Panel::IsRecordingSBuffer()
{
	return m_SurfaceCallRecordFrameTime == system()->GetFrameTime();
}

void Panel::FinishRecordSBuffer()
{
	SBufferFinishRecording();
}

void Panel::DrawFromSBuffer()
{
	if( cl_sbuffernodraw.GetBool() )
		return;

	//Msg("Name: %s, Class: %s, Panel: %d\n", GetName(), GetClassName(), this);
	//Msg("Draw time: %f, cur frame time: %f\n", m_SurfaceCallDrawFrameTime, system()->GetFrameTime() );
	if( m_SurfaceCallDrawFrameTime != system()->GetFrameTime() )
	{
		m_iNumSBufferCall = 0;
		m_SurfaceCallDrawFrameTime = system()->GetFrameTime();
	}
	...
	if( !m_SurfaceCallBuffer.IsValidIndex( m_iNumSBufferCall ) )
		return;

	//if( m_iNumSBufferCall > 0 )
	//	Msg("Rendering multiple times %d %d\n", m_iNumSBufferCall, GetParent() != NULL);

	//Msg("Drawing from sbuffer: %d\n", m_SurfaceCallBuffer.Count());
	for( int i = 0; i < m_SurfaceCallBuffer[m_iNumSBufferCall].Count(); i++ )
	{
		m_SurfaceCallBuffer[m_iNumSBufferCall][i]->Draw();
	}
}
#endif // USESURFACECALLBUFFER
...
{
	VGUI_UNIMPLEMENTED( Panel::SetShowDragHelper );
}

// Seems like a function to filter mouse input. Just always return true.
...
{
	m_LastNavDirection = ND_UP;
	return m_NavUp.Get();
}

Panel* Panel::NavigateDown()
{
	m_LastNavDirection = ND_DOWN;
	return m_NavDown.Get();
}

Panel* Panel::NavigateLeft()
{
	m_LastNavDirection = ND_LEFT;
	return m_NavLeft.Get();
}

Panel* Panel::NavigateRight()
{
	m_LastNavDirection = ND_RIGHT;
	return m_NavRight.Get();
}

void Panel::NavigateTo()
{
	KeyValues *msg = new KeyValues( "OnNavigateTo" );
...
{
	KeyValues *msg = new KeyValues( "OnNavigateFrom" );
	msg->SetString("panelName", GetName() );
	PostActionSignal( msg );
}
...
{
	VGUI_UNIMPLEMENTED( Panel::SetNavUp );
}

void Panel::SetNavDown( const char* controlName )
{
	VGUI_UNIMPLEMENTED( Panel::SetNavDown );
}

void Panel::SetNavLeft( const char* controlName )
{
	VGUI_UNIMPLEMENTED( Panel::SetNavLeft );
}

void Panel::SetNavRight( const char* controlName )
{
	VGUI_UNIMPLEMENTED( Panel::SetNavRight );
}
...
{
	VGUI_UNIMPLEMENTED( Panel::GetNavUpPanel );
	return NULL;
}

Panel* Panel::GetNavDownPanel()
{
	VGUI_UNIMPLEMENTED( Panel::GetNavDownPanel );
	return NULL;
}

Panel* Panel::GetNavLeftPanel()
{
	VGUI_UNIMPLEMENTED( Panel::GetNavLeftPanel );
	return NULL;
}

Panel* Panel::GetNavRightPanel()
{
	VGUI_UNIMPLEMENTED( Panel::GetNavRightPanel );
	return NULL;
}

Panel* Panel::GetNavUp( Panel *first )
{
	VGUI_UNIMPLEMENTED( Panel::GetNavUp );
	return NULL;
}
Panel* Panel::GetNavDown( Panel *first )
{
	VGUI_UNIMPLEMENTED( Panel::GetNavDown );
	return NULL;
}
Panel* Panel::GetNavLeft( Panel *first )
{
	VGUI_UNIMPLEMENTED( Panel::GetNavLeft );
	return NULL;
}
Panel* Panel::GetNavRight( Panel *first )
{
	VGUI_UNIMPLEMENTED( Panel::GetNavRight );
	return NULL;
}

void Panel::GetSizerMinimumSize(int &wide, int &tall)
{
	VGUI_UNIMPLEMENTED( Panel::GetSizerMinimumSize );
}

void Panel::GetSizerClientArea(int &x, int &y, int &wide, int &tall)
{
	VGUI_UNIMPLEMENTED( Panel::GetSizerClientArea );
}

void Panel::PinToSibling( const char *pszSibling, PinCorner_e pinOurCorner, PinCorner_e pinSibling )
{
	VGUI_UNIMPLEMENTED( Panel::PinToSibling );
}

void Panel::UpdateSiblingPin( void )
{
	VGUI_UNIMPLEMENTED( Panel::UpdateSiblingPin );
}
...
void Panel::PostMessageToAllSiblings( KeyValues *msg, float delaySeconds )
{
...
{
	return (_buildModeFlags & BUILDMODE_SAVE_PERCENTAGE) != 0;
}
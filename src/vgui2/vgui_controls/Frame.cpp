	...
	public:
		...
		void OnMousePressed(MouseCode code)
		{
			...
			{ 
				...
				VPANEL focus = input()->GetCalculatedFocus();
...
// Purpose: Constructor
//-----------------------------------------------------------------------------
Frame::Frame(Panel *parent, const char *panelName, bool showTaskbarIcon, bool bPopup) : EditablePanel(parent, panelName)
{
	// frames start invisible, to avoid having window flicker in on taskbar
	SetVisible(false);
	MakePopup(showTaskbarIcon);
	ipanel()->SetPopup(GetVPanel(), bPopup);
...
{
	VGUI_UNIMPLEMENTED(Frame::GetSizerClientArea);
}

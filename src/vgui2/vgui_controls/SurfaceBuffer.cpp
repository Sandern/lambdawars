...
#include <vgui_controls/SurfaceBuffer.h>
...
/*
bool ShouldRecordSurfaceBuffer( CUtlVector<vgui::BaseBufferCall *> *pCallBuffer, bool bShouldRecordBuffer )
{
	if( bShouldRecordBuffer || cl_recordsurfacebuffer.GetBool() == true )
	{
		// Already got an active buffer.
		// Just add the calls from this panel to the panel with the active buffer
		if( g_pActiveSurfaceBuffer )
			return true;
		...
		g_pActiveSurfaceBuffer->PurgeAndDeleteElements();
	...
	pRealVGuiSurface = g_pVGuiSurface;
	g_pActiveSurfaceBuffer = pCallBuffer;
	for( int i = 0; i < (*g_pActiveSurfaceBuffer).Count(); i++ )
	{
		(*g_pActiveSurfaceBuffer)[i]->Draw();
	}

	return false;
}

void FinishSurfaceBuffer()
{
	if( g_pActiveSurfaceBuffer )
	{
		// Restore
...
{ 
	g_pActiveSurfaceBuffer->AddToTail( new BufferCallSetVisible(panel, true) );
	g_pActiveSurfaceBuffer->AddToTail( 
		new BufferCallPaintTraverse(panel) //new BufferCallArg1<void, VPANEL>( &ISurface::PaintTraverse, panel )
	);
	g_pActiveSurfaceBuffer->AddToTail( new BufferCallSetVisible(panel, false) );

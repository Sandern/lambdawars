...
ConVar nav_coplanar_slope_limit( "nav_coplanar_slope_limit", "0.99", FCVAR_CHEAT|FCVAR_REPLICATED );
ConVar nav_coplanar_slope_limit_displacement( "nav_coplanar_slope_limit_displacement", "0.7", FCVAR_CHEAT|FCVAR_REPLICATED );
ConVar nav_split_place_on_ground( "nav_split_place_on_ground", "0", FCVAR_CHEAT|FCVAR_REPLICATED, "If true, nav areas will be placed flush with the ground when split." );
...
ConVar nav_max_view_distance( "nav_max_view_distance", "0", FCVAR_CHEAT|FCVAR_REPLICATED, "Maximum range for precomputed nav mesh visibility (0 = default 1500 units)" );
ConVar nav_update_visibility_on_edit( "nav_update_visibility_on_edit", "0", FCVAR_CHEAT|FCVAR_REPLICATED, "If nonzero editing the mesh will incrementally recompue visibility" );
ConVar nav_potentially_visible_dot_tolerance( "nav_potentially_visible_dot_tolerance", "0.98", FCVAR_CHEAT|FCVAR_REPLICATED );
ConVar nav_show_potentially_visible( "nav_show_potentially_visible", "0", FCVAR_CHEAT|FCVAR_REPLICATED, "Show areas that are potentially visible from the current nav area" );
...
ConVar nav_selected_set_color( "nav_selected_set_color", "255 255 200 96", FCVAR_CHEAT|FCVAR_REPLICATED, "Color used to draw the selected set background while editing.", false, 0.0f, false, 0.0f, SelectedSetColorChaged );
ConVar nav_selected_set_border_color( "nav_selected_set_border_color", "100 100 0 255", FCVAR_CHEAT|FCVAR_REPLICATED, "Color used to draw the selected set borders while editing.", false, 0.0f, false, 0.0f, SelectedSetColorChaged );
#endif // CLIENT_DLL
...
//--------------------------------------------------------------------------------------------------------------
/**
 * On the fly/nodeless version of  the above
 */
void CNavArea::FinishMergeOnTheFly( CNavArea *adjArea )
{
	// update extent
	Vector vCornerNWAdj = adjArea->GetCorner( NORTH_WEST );
	Vector vCornerSEAdj = adjArea->GetCorner( SOUTH_EAST );

	m_nwCorner.x = MIN(m_nwCorner.x, vCornerNWAdj.x );
	m_nwCorner.y = MIN(m_nwCorner.y, vCornerNWAdj.y );

	m_seCorner.x = MAX(m_seCorner.x, vCornerSEAdj.x );
	m_seCorner.y = MAX(m_seCorner.y, vCornerSEAdj.y );
...
void CNavArea::ComputeSemiPortal( const CNavArea *to, NavDirType dir, Vector *center, float *halfWidth ) const
{
	if ( dir == NORTH || dir == SOUTH )
	{
		if ( dir == NORTH )
 ...
 */
void CNavArea::ComputeClosestPointInPortal( const CNavArea *to, NavDirType dir, const Vector &fromPos, Vector *closePos, float margin ) const
{
...
{
	return m_attributeFlags & NAV_MESH_NAV_BLOCKER;

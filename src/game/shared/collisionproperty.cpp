...
//		DEFINE_FIELD( m_pOuter, FIELD_CLASSPTR ),
		...
		DEFINE_FIELD( m_flRadius2D, FIELD_FLOAT ),
...
// HL2Wars: Used a lot in the unit navigator, so we cache it above
//-----------------------------------------------------------------------------
...
void CCollisionProperty::MarkSurroundingBoundsDirty()
{
	...
	if ( m_pOuter->entindex() == 0 && !m_pOuter->IsEFlagSet( EFL_SERVER_ONLY ) )
		return;
...
void CCollisionProperty::UpdateServerPartitionMask( )
{
	...
	if( !m_pOuter->IsEFlagSet( EFL_SERVER_ONLY ) ) // Allow server only entities to have collision
	{

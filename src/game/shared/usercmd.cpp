...
void WriteUsercmd( bf_write *buf, const CUserCmd *to, const CUserCmd *from )
{
...
#ifdef HL2WARS_DLL
	if( to->m_vMouseAim != from->m_vMouseAim )
	{
		buf->WriteOneBit( 1 );
		buf->WriteBitVec3Normal( to->m_vMouseAim );
	}
	else
	{
		buf->WriteOneBit( 0 );
	...
	if( to->m_vCameraOffset != from->m_vCameraOffset )
	{
		buf->WriteOneBit( 1 );
		buf->WriteBitVec3Coord( to->m_vCameraOffset );
	}
	else
	{
		buf->WriteOneBit( 0 );
	...
	if( to->vecmovetoposition != from->vecmovetoposition )
	{
		buf->WriteOneBit( 1 );
		buf->WriteBitVec3Coord( to->vecmovetoposition );
	}
	else
	{
		buf->WriteOneBit( 0 );
	...
	if( to->directmove != from->directmove )
	{
		buf->WriteOneBit( 1 );
		buf->WriteOneBit( to->directmove );
	}
	else
	{
		buf->WriteOneBit( 0 );
...
void ReadUsercmd( bf_read *buf, CUserCmd *move, CUserCmd *from )
{
	...
	*move = *from;
	...
	if ( buf->ReadOneBit() )
	{
		buf->ReadBitVec3Normal( move->m_vMouseAim );
	}

	if ( buf->ReadOneBit() )
	{
		buf->ReadBitVec3Coord( move->m_vCameraOffset );
	}

	if ( buf->ReadOneBit() )
	{
		buf->ReadBitVec3Coord( move->vecmovetoposition );
	}

	if ( buf->ReadOneBit() )
	{
		move->directmove = buf->ReadOneBit();
	}
#endif //HL2WARS_DLL

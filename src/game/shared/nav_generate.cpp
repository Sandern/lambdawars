...
const char *CNavMesh::GetPlayerSpawnName( void ) const
{
	...
	// default value
	return "info_start_wars"; //"info_player_start";
}
...
class CreateNavAreaTestConnection
{
public:
	bool operator() ( CNavArea *targetArea )
	{
		return false;
	}
};

int CNavMesh::CreateNavArea( const Vector &corner, const Vector &otherCorner )
{
	// Create and construct a new area
	CNavArea *newArea = TheNavMesh->CreateArea();
	if (newArea == NULL)
	{
		Warning( "CreateNavArea: Out of memory\n" );
		return -1;
	}
	
	newArea->Build( corner, otherCorner );

	TheNavAreas.AddToTail( newArea );
	TheNavMesh->AddNavArea( newArea );

	// Connect to nearby areas
	CreateNavAreaTestConnection func;
	for( int i=0; i < NUM_DIRECTIONS; i++ )
		TheNavMesh->StitchAreaIntoMesh<CreateNavAreaTestConnection>( newArea, (NavDirType)i, func );

	return newArea->GetID();
}

int CNavMesh::CreateNavArea( const Vector &nwCorner, const Vector &neCorner, const Vector &seCorner, const Vector &swCorner )
{
	// Create and construct a new area
	CNavArea *newArea = TheNavMesh->CreateArea();
	if (newArea == NULL)
	...
	newArea->Build( nwCorner, neCorner, seCorner, swCorner );

...
// Input  : list - 
//-----------------------------------------------------------------------------
void GetAllParticleFiles( CUtlVector<CUtlString>& list, const char *pDirectory="particles/" )
{
	// Read all files from the particles folder
	char ext[8];
	...
	Q_snprintf(wildCard, _MAX_PATH, "%s*", pDirectory);
	pFileName = filesystem->FindFirstEx(wildCard, NULL, &fh);
	//Msg("Parsing particle directory %s\n", wildCard);
	
	while( pFileName )
	{
		Q_ExtractFileExtension(pFileName, ext, 8);
		//Msg("Parsing file: %s | ext: %s\n", pFileName, ext);
		if( Q_strncmp(pFileName, ".", 1) == 0 || Q_strncmp(pFileName, "..", 2) == 0 )
		{
			// do nothing
		}
		else if( filesystem->FindIsDirectory(fh) )
		{
			char subdir[_MAX_PATH];
			Q_snprintf(subdir, _MAX_PATH, "%s%s/", pDirectory, pFileName);
			GetAllParticleFiles( list, subdir );
		}
		else if( Q_strncmp(ext, "pcf", 3) == 0 )
		{
			Q_snprintf(fullPath, _MAX_PATH, "%s%s", pDirectory, pFileName);
			//Msg("Parsing %s\n", fullPath);
			list.AddToTail(fullPath);
		}

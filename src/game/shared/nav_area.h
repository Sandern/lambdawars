...
public:
	...
	void ComputeClosestPointInPortal( const CNavArea *to, NavDirType dir, const Vector &fromPos, Vector *closePos, float margin=GenerationStepSize ) const; // compute closest point within the "portal" between to adjacent areas
	NavDirType ComputeDirection( Vector *point ) const;			// return direction from this area to the given point

	void ComputeSemiPortal( const CNavArea *to, NavDirType dir, Vector *center, float *halfWidth ) const;
	...
	static void SetCachedPath( CUtlSymbol id, int iStartAreaID, int iGoalAreaID, int iClosestAreaID )
	{
		...
		m_iLastStartAreaID = iStartAreaID;
		m_iLastGoalAreaID = iGoalAreaID;
...
private:
	...
	*/
	...
	static CUtlSymbol m_LastCachedPathID;
	static int m_iLastStartAreaID;
	static int m_iLastGoalAreaID;
	static int m_iLastClosestAreaID;
	...
	void FinishMergeOnTheFly( CNavArea *adjArea );

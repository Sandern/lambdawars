...
const float GenerationStepSize = 25.0f;			// (30) was 20, but bots can't fit always fit
const float JumpHeight = 41.8f;					// if delta Z is less than this, we can jump up on it
const float JumpCrouchHeight = 512.0f;//64.0f;			// (48) if delta Z is less than or equal to this, we can jumpcrouch up on it

// There are 3 different definitions of StepHeight throughout the code, waiting to produce bugs if the 18.0 is ever changed.
const float StepHeight = 24.0f;					///< if delta Z is greater than this, we have to jump to get up

// HL2Wars settings
const float DeathDrop = 720.0f;				// Setup a big drop value, units check deathdrop in pathfinding
const float ClimbUpHeight = 512.0f;			// Climbing
const float CliffHeight = 512.0f;			// height which we consider a significant cliff which we would not want to fall off of

...
void CBaseCombatWeapon::WeaponSound( WeaponSound_t sound_type, float soundtime /* = 0.0f */ )
{
	...
	{
		const Vector &origin = GetOwner() ? GetOwner()->GetAbsOrigin() : GetAbsOrigin();
		...
		if ( GetOwner() )
		{
			...
			EmitSound( filter, GetOwner()->entindex(), shootsound, &origin, soundtime ); 
		...
		else
		{
			...
			EmitSound( filter, entindex(), shootsound, &origin, soundtime ); 
		}
	}
}

...
CBaseEntity::~CBaseEntity( )
{
...
#endif // ENABLE_PYTHON
	{
		...
		if( (m_nFOWFlags & FOWFLAG_HIDDEN) || (m_nFOWFlags & FOWFLAG_NOTRANSMIT) )
...
{
	if ( (!edict() && !IsEFlagSet( EFL_SERVER_ONLY )) || m_takedamage < DAMAGE_YES )
		return 0;
...
int CBaseEntity::OnTakeDamage( const CTakeDamageInfo &info )
{
	...
	if ( (!edict() && !IsEFlagSet( EFL_SERVER_ONLY )) || !m_takedamage )
		return 0;
...
// Purpose: 
//------------------------------------------------------------------------------
void CBaseEntity::SetOwnerNumber( int owner_number ) 
{ 
...
// Purpose: 
//------------------------------------------------------------------------------
void CBaseEntity::InputChangeOwnerNumber( inputdata_t &inputdata )
{ 
	int new_owner = inputdata.value.Int();
	SetOwnerNumber(new_owner); 
}
...
// Purpose: 
//------------------------------------------------------------------------------
void CBaseEntity::FOWForceUpdate( int iPlayerIndex )
{
...
void CBaseEntity::DestroyPyInstance()
{
	...
	// Do things we need to do before ehandle goes null
	g_bDisableEhandleAccess = false;
	CBaseEntity::PhysicsRemoveTouchedList( this );
	CBaseEntity::PhysicsRemoveGroundList( this );
	DestroyAllDataObjects();
...
//------------------------------------------------------------------------------
// Purpose: Check our __class__ instance by comparing it to the module __class__
//			instance. If it does not match it means it got reloaded. Remove ourself.
//------------------------------------------------------------------------------
bool CBaseEntity::ValidatePyInstance()
{
	try
	{
		// Get our class instance by importing the module where we are defined
		// Then get the class definition to ensure we have the newest one
		boost::python::object myclassref = GetPyInstance().attr("__class__");
		const char *pModName = boost::python::extract<const char *>( myclassref.attr("__module__") );
		boost::python::object classref = SrcPySystem()->Import(pModName).attr(myclassref.attr("__name__"));
		if( classref.ptr() != myclassref.ptr() )
		{
			DevMsg(2, "ValidatePyInstance: Removing entity with invalid class instance\n");
			Remove();
			SrcPySystem()->Import("unitmgr").attr("RefreshUnitList")();
			return false;
		}
	}
	catch(boost::python::error_already_set &)

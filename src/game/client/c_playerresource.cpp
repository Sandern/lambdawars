...
BEGIN_PREDICTION_DATA( C_PlayerResource )
	...
	DEFINE_PRED_ARRAY( m_iOwnerNumbers, FIELD_INTEGER, MAX_PLAYERS+1, FTYPEDESC_PRIVATE ),
...
// Purpose: 
//-----------------------------------------------------------------------------
int	C_PlayerResource::GetOwnerNumber( int iIndex )
{
	if ( !IsConnected( iIndex ) )
		return 0;

	return m_iOwnerNumbers[iIndex];
}

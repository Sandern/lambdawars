...
CSDK_Background_Movie *g_pBackgroundMovie = NULL;

CSDK_Background_Movie* SDKBackgroundMovie()
{
	if ( !g_pBackgroundMovie )
	{
		g_pBackgroundMovie = new CSDK_Background_Movie();
	}
	return g_pBackgroundMovie;
}

CSDK_Background_Movie::CSDK_Background_Movie()
{
...
void CSDK_Background_Movie::ClearCurrentMovie()
{
#ifdef ASW_BINK_MOVIES
	if ( m_nBIKMaterial != BIKMATERIAL_INVALID )
	{
...
int CSDK_Background_Movie::SetTextureMaterial()
{
#ifdef ASW_BINK_MOVIES
	if ( m_nBIKMaterial == BIKMATERIAL_INVALID )
		return -1;
...
{
	if ( engine->IsConnected() && HL2WarsGameRules() )
	{
...
ConVar sdk_background_color( "sdk_background_color", "0 0 0 128", FCVAR_NONE, "Color of background tinting in briefing screens" );
		...
		case NB_BACKGROUND_TRANSPARENT_BLUE:
			{
				m_pBackground->SetVisible( true );
				m_pBackgroundImage->SetVisible( false );
				m_pBackground->SetBgColor( sdk_background_color.GetColor() );
				break;
			}
...
void CNB_Header_Footer::PaintBackground()
{
	...
	if ( m_bMovieEnabled && SDKBackgroundMovie() )
	{

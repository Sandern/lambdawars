...
void GameSettings::Activate()
{
	...
	if ( m_drpGameType )
	{
		...
		if ( szGameMode )
		{
			...
			{
				m_drpGameType->SetCurrentSelection( "Level" );
			}	
			else if( !Q_stricmp( szGameMode, "swarmkeeper" ) )
			{
				m_drpGameType->SetCurrentSelection( "Level" );
			}	
		}
...
void GameSettings::ShowMissionSelect()
{
	...
	if ( m_pSettings )
	{
		...
		{
			CNB_Select_Level_Panel *pPanel = new CNB_Select_Level_Panel( this, "Select_Level_Panel", "swarmkeepermaps.txt" );
			pPanel->MoveToFront();

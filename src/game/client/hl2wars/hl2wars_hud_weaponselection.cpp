...
void CHudWeaponSelection::SelectSlot( int iSlot )
{
	...
	m_flLastKeyPressedTime = gpGlobals->curtime;
	m_iSlotDoublePressedTime = iSlot-1;

	CBaseEntity *pControlledUnit = pPlayer->GetControlledUnit();
	if( pControlledUnit && pControlledUnit->GetIUnit() )
	{
		if( pControlledUnit->GetIUnit()->SelectSlot( iSlot ) )
			return;
	}
	...
	if( pPlayer->m_nButtons & IN_SPEED )
	{
		...
		engine->ServerCmd( VarArgs( "make_group %d %d", iSlot, 0 ) );
	}
	else if( pPlayer->m_nButtons & IN_DUCK )
	{
		pPlayer->MakeCurrentSelectionGroup( iSlot, true );
		engine->ServerCmd( VarArgs( "make_group %d %d", iSlot, 1 ) );
	}
	else 
	{
...
void CHudWeaponSelection::LevelInit()
{
	...
	m_iSlotDoublePressedTime = -1;
	m_flLastKeyPressedTime = -1;
}

//-----------------------------------------------------------------------------
// Purpose: 
//-----------------------------------------------------------------------------
ConVar cl_numkeys_doublepresstime("cl_numkeys_doublepresstime", "0.2");
int	CHudWeaponSelection::KeyInput( int down, ButtonCode_t keynum, const char *pszCurrentBinding )
{
	...
	for( int i = 0; i < 10; i++ )
	{
		...
		if( m_iSlotDoublePressedTime != i || m_flLastKeyPressedTime == -1 )
			continue;
		...
		if( pszCurrentBinding && gpGlobals->curtime - m_flLastKeyPressedTime < cl_numkeys_doublepresstime.GetFloat() )
		{
			if( Q_strncmp( pszCurrentBinding, buf, 6 ) == 0 )
			{
				...
				DevMsg("Snapping camera to group %d\n", m_iSlotDoublePressed);
				pPlayer->CamFollowGroup( pPlayer->GetGroup( m_iSlotDoublePressed ) );
				return 0;
			}
		}
	}

	m_iSlotDoublePressedTime = -1; // Always invalidate if another key was pressed

...
#include "wars/VDropDownMenu.h"
#include "wars/VFlyoutMenu.h"
...
extern ConVar cl_active_config;
...
{
	char configdefault[MAX_PATH];
	Q_snprintf( configdefault, MAX_PATH, "cfg/%s_default.cfg", cl_active_config.GetString());

	//FileHandle_t fh = g_pFullFileSystem->Open( "cfg/config_default.cfg", "rb" );
	FileHandle_t fh = g_pFullFileSystem->Open( configdefault, "rb" );
...
void COptionsSubKeyboard::OnThink()
{
	...
	if( !m_drpConfig )
	{
		m_drpConfig = dynamic_cast< BaseModUI::DropDownMenu* >( FindChildByName( "DrpConfig" ) );

		if( m_drpConfig )
		{
			m_drpConfig->SetCurrentSelection( "#GameUI_RTS" );
		}
	}

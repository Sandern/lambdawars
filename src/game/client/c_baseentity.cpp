	...
	case SHOULDTRANSMIT_END:
		{
			...
			if( (GetFOWFlags() & FOWFLAG_KEEPCOLINFOW) == 0 )
			{
...
void C_BaseEntity::OnDataChanged( DataUpdateType_t type )
{
	...
	if( m_iOldOwnerNumber != m_iOwnerNumber )
	{
		OnChangeOwnerNumberInternal(m_iOldOwnerNumber);
		m_iOldOwnerNumber = m_iOwnerNumber;
	}

	if( m_nOldFOWFlags != m_nFOWFlags )
	{
		if( !(m_nOldFOWFlags & FOWFLAG_UPDATER) && (m_nFOWFlags & FOWFLAG_UPDATER) )
			FogOfWarMgr()->AddFogUpdater( m_iOwnerNumber, this );
		else if( (m_nOldFOWFlags & FOWFLAG_UPDATER) && !(m_nFOWFlags & FOWFLAG_UPDATER) )
			FogOfWarMgr()->RemoveFogUpdater( m_iOwnerNumber, this );

		if( (!(m_nOldFOWFlags & FOWFLAG_HIDDEN) && (m_nFOWFlags & FOWFLAG_HIDDEN)) || 
				(!(m_nOldFOWFlags & FOWFLAG_NOTRANSMIT) && (m_nFOWFlags & FOWFLAG_NOTRANSMIT)) )
			FogOfWarMgr()->AddFogEntity(this);
		else if( ((m_nOldFOWFlags & FOWFLAG_HIDDEN) && !(m_nFOWFlags & FOWFLAG_HIDDEN)) ||
				((m_nOldFOWFlags & FOWFLAG_NOTRANSMIT) && !(m_nFOWFlags & FOWFLAG_NOTRANSMIT)) )
			FogOfWarMgr()->RemoveFogEntity(this);

		m_nOldFOWFlags = m_nFOWFlags;
		UpdateVisibility();
	}
}
...
const char *C_BaseEntity::GetClassname( void )
{
	...
	// Temp, until we can make a pred map in python.
	if( m_iszOverrideClassname != NULL_STRING )
	{
		Q_strncpy( outstr, STRING(m_iszOverrideClassname), sizeof( outstr ) );
		gotname = true;
	}

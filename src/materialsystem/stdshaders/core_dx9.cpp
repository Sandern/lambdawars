...
BEGIN_VS_SHADER( Core_DX90, 
	...
	BEGIN_SHADER_PARAMS
		...
		SHADER_PARAM( SPHEREDIAMETER, SHADER_PARAM_TYPE_FLOAT, "430.0f", "" );
		SHADER_PARAM( SPHEREORIGIN, SHADER_PARAM_TYPE_VEC3, "[0 0 0]", "" );
		SHADER_PARAM( SPHEREOFFSET, SHADER_PARAM_TYPE_VEC3, "[0 0 0]", "" );
	END_SHADER_PARAMS
		...
		IShaderDynamicAPI* pShaderAPI, int nPass, VertexCompressionType_t vertexCompression ) 
	{
		...
		DYNAMIC_STATE
		{
			...
			const float *origin = params[SPHEREORIGIN]->GetVecValue();
			const float *offset = params[SPHEREOFFSET]->GetVecValue();
			static Vector s_SphereOrigin;
			s_SphereOrigin[0] = origin[0] + offset[0];
			s_SphereOrigin[1] = origin[1] + offset[1];
			s_SphereOrigin[2] = origin[2] + offset[2];
			pShaderAPI->SetPixelShaderConstant( 13, s_SphereOrigin.Base(), 3 );

			SetPixelShaderConstant( 12, SPHEREDIAMETER );
		}
		Draw();
	}

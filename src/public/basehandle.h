...
inline void CBaseHandle::Init( int iEntry, int iSerialNumber )
{
	...
	if( iEntry >= ORI_NUM_ENT_ENTRIES )
	{
		m_Index = iEntry | (iSerialNumber << NUM_SERIAL_NUM_SHIFT_BITS);
		m_Index |= EXT_EXTEND_TEST_BIT;
	}
	else
	{
		m_Index = iEntry | (iSerialNumber << ORI_NUM_SERIAL_NUM_SHIFT_BITS);
		m_Index &= ~EXT_EXTEND_TEST_BIT;
	}
#else
	m_Index = iEntry | (iSerialNumber << NUM_SERIAL_NUM_SHIFT_BITS);
...
#ifdef USE_INCREASED_NON_NETWORKED
	if( m_Index & EXT_EXTEND_TEST_BIT )
		return (m_Index & ~EXT_EXTEND_TEST_BIT) & ENT_ENTRY_MASK;
	return m_Index & ORI_ENT_ENTRY_MASK;
#else
...
#ifdef USE_INCREASED_NON_NETWORKED
	if( m_Index & EXT_EXTEND_TEST_BIT )
		return (m_Index & ~EXT_EXTEND_TEST_BIT) >> NUM_SERIAL_NUM_SHIFT_BITS;
	return m_Index >> ORI_NUM_SERIAL_NUM_SHIFT_BITS;
#else

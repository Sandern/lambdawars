...
calculate_path(void)
{
	...
	char pythondir[_MAX_PATH];
	char dllsdir[_MAX_PATH];
	char libdir[_MAX_PATH];
	char platwindir[_MAX_PATH];
	char libtkdir[_MAX_PATH];

	// Get all dirs
	getmoddir(moddir, _MAX_PATH);

#if 0
	snprintf(pythondir, _MAX_PATH, "%spython\\.\0", moddir);
	snprintf(dllsdir, _MAX_PATH, "%spython\\DLLs\0", moddir);
	snprintf(libdir, _MAX_PATH, "%spython\\lib\0", moddir);
	snprintf(platwindir, _MAX_PATH, "%spython\\plat-win\0", moddir);
	snprintf(libtkdir, _MAX_PATH, "%spython\\lib-tk\0", moddir);
#endif

#ifdef MS_WINDOWS
	snprintf(pythondir, _MAX_PATH, "%spython\0", moddir);
	if( SrcPyIsClient() )
		snprintf(dllsdir, _MAX_PATH, "%spython\\ClientDLLs\0", moddir);
	else
		snprintf(dllsdir, _MAX_PATH, "%spython\\DLLs\0", moddir);
	snprintf(libdir, _MAX_PATH, "%spython\\Lib\0", moddir);
	snprintf(platwindir, _MAX_PATH, "%spython\\plat-win\0", moddir);
	snprintf(libtkdir, _MAX_PATH, "%spython\\Lib\\lib-tk\0", moddir);
#else
	snprintf(pythondir, _MAX_PATH, "%spython\0", moddir);
	snprintf(dllsdir, _MAX_PATH, "%spython/DLLs\0", moddir);
	snprintf(libdir, _MAX_PATH, "%spython/Lib\0", moddir);
	...
	// The length of the path is the dirs + the number of required delims + one ending character
	bufsz = strlen(pythondir) + strlen(dllsdir) + strlen(libdir) + 
		strlen(platwindir) + strlen(libtkdir) + 6;

	// Allocate buf
	module_search_path = buf = malloc(bufsz);
	if (buf == NULL) {
		/* We can't exit, so print a warning and limp along */
		SrcWarning("Can't malloc dynamic PYTHONPATH.\n");
		SrcWarning("Using default static path.\n");
		module_search_path = PYTHONPATH;
		return;
	}

	// Copy all paths into the buf
	strcpy(buf, pythondir);
	buf = strchr(buf, '\0');
	*buf++ = DELIM;
	strcpy(buf, dllsdir);
	buf = strchr(buf, '\0');

...
static PyObject*
builtin_Msg(PyObject *self, PyObject *args)
{
	SrcMsg( PyString_AsString(args) );
	Py_RETURN_NONE;
}

PyDoc_STRVAR(Msg_doc,
			 "Print a message in the console. Equals print().");


static PyObject*
builtin_PrintWarning(PyObject *self, PyObject *args)
{
	SrcWarning( PyString_AsString(args) );
	Py_RETURN_NONE;
}

PyDoc_STRVAR(PrintWarning_doc,
			 "Print a message in red");

static PyObject*
builtin_DevMsg(PyObject *self, PyObject *args)
{
	PyObject *level = NULL, *msg = NULL;
	char *devmsg = NULL;

	if (!PyArg_UnpackTuple(args, "DevMsg", 2, 2, &level, &msg))
		return NULL;

	devmsg = PyString_AsString(msg);
	if( devmsg )
		SrcDevMsg( PyInt_AsLong(level), PyString_AsString(msg) );
	Py_RETURN_NONE;
}

PyDoc_STRVAR(DevMsg_doc,
			 "Only prints te message if developer >= level.");
...
static PyMethodDef builtin_methods[] = {
	...
	{"PrintWarning", builtin_PrintWarning,	METH_O, PrintWarning_doc},
	{"Msg",		builtin_Msg,		METH_O, Msg_doc},
	{"DevMsg",		builtin_DevMsg,		METH_VARARGS, DevMsg_doc},
...
#define SETBUILTIN(NAME, OBJECT) \
	...
	SETBUILTIN("gpGlobals",		Py_None);
	SETBUILTIN("isclient",		Py_False);
	SETBUILTIN("isserver",		Py_False);

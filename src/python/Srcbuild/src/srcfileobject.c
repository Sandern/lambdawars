...
open_the_file(PyFileObject *f, char *name, char *mode)
{
	...
	/* Fail if not in the mod directory */
	if( SrcPyGetFullPathSilent(PyString_AsString(f->f_name), fullPath, _MAX_PATH) == 0 ) {
		PyErr_OutsideModFolder();
		return NULL;
	}
	f->f_name = PyString_FromString(fullPath);
	name = PyString_AsString(f->f_name);
